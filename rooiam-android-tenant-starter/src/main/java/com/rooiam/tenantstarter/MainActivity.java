package com.rooiam.tenantstarter;
...
public final class MainActivity extends Activity {
    ...
    private int brandColor() { return Color.parseColor(BuildConfig.BRAND_COLOR); }
    ...
    private void home() {
        ...
        ScrollView scroll = new ScrollView(this); scroll.setFillViewport(true); scroll.setBackgroundColor(0xFFF8F6FC); scroll.addView(layout); content(scroll);
        ImageView logo = new ImageView(this);
        logo.setImageResource(getResources().getIdentifier(BuildConfig.LOGO_RESOURCE, "drawable", getPackageName()));
        logo.setContentDescription(getString(R.string.app_name) + " logo");
        int logoPixels = (int) (96 * getResources().getDisplayMetrics().density);
        LinearLayout.LayoutParams logoSize = new LinearLayout.LayoutParams(logoPixels, logoPixels); logoSize.bottomMargin = 20; layout.addView(logo, logoSize);
        TextView title = new TextView(this); title.setText(getString(R.string.app_name)); title.setTextSize(28); title.setTextColor(0xFF262039); title.setTypeface(null, android.graphics.Typeface.BOLD); layout.addView(title);
        TextView subtitle = new TextView(this); subtitle.setText("Scan. Match. Approve."); subtitle.setTextSize(16); subtitle.setTextColor(brandColor()); subtitle.setPadding(0,4,0,16); layout.addView(subtitle);
        status = new TextView(this); status.setText("Enroll your phone, then approve only sign-ins you started."); status.setTextColor(0xFF342F45); status.setPadding(20,20,20,20); status.setBackground(surface(Color.WHITE, BuildConfig.CORNER_RADIUS_DP)); layout.addView(status);
        server = new TextView(this); server.setText("Connected to " + BuildConfig.API_ORIGIN); server.setTextColor(0xFF6B6578); server.setPadding(0,18,0,12); layout.addView(server);
    ...
    private void button(String label, Runnable action) {
        ...
        b.setTextColor(Color.WHITE); b.setBackground(surface(brandColor(), BuildConfig.CORNER_RADIUS_DP));
        LinearLayout.LayoutParams spacing = new LinearLayout.LayoutParams(-1, -2); spacing.bottomMargin = 12;
        b.setOnClickListener(v -> action.run()); layout.addView(b, spacing);
    }
    private void startScanner() {
        if (busy || isFinishing() || isDestroyed()) return;
        try {
            Protocol.origin(BuildConfig.API_ORIGIN, BuildConfig.DEBUG);
    ...
    private void work(Work action, Runnable success) {
        ...
        configuredProject = BuildConfig.CLOUD_PROJECT_NUMBER;
    ...
    private String serverOrigin() {
        return Protocol.origin(BuildConfig.API_ORIGIN, BuildConfig.DEBUG);
    }
            ...
            new AlertDialog.Builder(this).setTitle("Open your sign-in link").setView(input)
                .setPositiveButton("Open", (d,w) -> {
                    try {
                        ...
                        web.loadUrl(Protocol.emailLink(link.toASCIIString(), frontend, BuildConfig.API_ORIGIN, BuildConfig.DEBUG));
    ...
    private String enroll() throws Exception {
        ...
        if (!configuredProject.trim().isEmpty() && !configuredProject.equals("0")) {
    ...
    private void preview(String qr) {
        ...
        try {
            Protocol.parseQr(qr, BuildConfig.API_ORIGIN, BuildConfig.DEBUG);
            pendingQr = qr;
