...
// 0.7: protect only full-access owner keys while reduced admin keys stay usable.
reject(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { mode: 'invalid' } }))
reject(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { mode: 'off', required: true } }))
assert.equal(expect(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { mode: 'owner_keys' } }), 200).mode, 'owner_keys')
assert.equal(expect(await request('/orgs/current/api-key-phone-policy', { cookie: owner }), 200).required, false)
const adminKeyWithOwnerPolicy = expect(await request('/orgs/current/api-keys', { method: 'POST', cookie: owner, body: keyBody }), 201)
expect(await request('/orgs/current/api-keys/' + adminKeyWithOwnerPolicy.key.id, { method: 'DELETE', cookie: owner }), 200)
expect(await request('/orgs/current/action-approvals', { method: 'POST', cookie: owner, body: keyBody }), 409)
reject(await request('/orgs/current/api-keys', { method: 'POST', cookie: owner, body: { ...keyBody, permission_preset: 'workspace_owner' } }))
expect(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { required: true } }), 200)
assert.equal(expect(await request('/orgs/current/api-key-phone-policy', { cookie: owner }), 200).mode, 'all_keys')
...
// A workspace admin can read the policy but cannot change it, even though
// admins may create reduced API keys. A stale owner session also cannot edit it.
const otherUserId = expect(await request('/identity/me', { cookie: otherUser.cookie }), 200).id
sql(`INSERT INTO organization_members (organization_id, user_id) VALUES ('${orgId}', '${otherUserId}')`)
sql(`INSERT INTO member_roles (member_id, role_id) SELECT om.id, r.id FROM organization_members om CROSS JOIN roles r WHERE om.organization_id = '${orgId}' AND om.user_id = '${otherUserId}' AND r.code = 'admin' AND r.is_system = TRUE`)
expect(await request('/orgs/switch', { method: 'POST', cookie: otherUser.cookie, body: { organization_id: orgId } }), 200)
expect(await request('/orgs/current/api-key-phone-policy', { cookie: otherUser.cookie }), 200)
expect(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: otherUser.cookie, body: { mode: 'off' } }), 403)
sql(`UPDATE sessions SET created_at = NOW() - interval '11 minutes' WHERE user_id = '${me.id}'`)
expect(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { mode: 'off' } }), 403)
...
expect(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { required: false } }), 200)
reject(await request('/orgs/current/api-key-phone-policy', { method: 'PUT', cookie: owner, body: { mode: 'owner_keys' } }))
console.log('PASS: off/owner-only/all-key policy modes, legacy boolean writes, direct-route enforcement, bindings, signature purpose, exact payload, replay, deny, cancel, race, key limit, policy/role change, expiry, recent-sign-in device revocation, and one-time key')
