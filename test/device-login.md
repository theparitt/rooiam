...
Enable the platform switch, then opt in each intended workspace through its session policy screen. Keep verified/vendor attestation policies and app allowlists appropriate to production. Revoking a device requires a session created within the previous 10 minutes; it stops new approvals and atomically rejects approved unfinished login intents and cancels unfinished API-key approvals. Revoke existing browser sessions separately when responding to compromise. The [lost-phone and replacement guide](../docs/reference/16_lost_phone_and_replacement.md) covers the user path. Restore server backups using existing procedures, but never restore phone private keys from app backups. Restore drills must account for already-consumed intents/sessions to avoid replaying old state.
...
The SDK is a standalone Gradle project in `rooiam-sdk/android`; the reference app is in `rooiam-examples/example-5-android-reference-app`. From the SDK directory, run `./gradlew testDebugUnitTest lintDebug assembleDebugAndroidTest`. From the example directory, run `./gradlew assembleDebug lintDebug`. Use the [SDK integration guide](../rooiam-sdk/android/README.md) for publication and consumption. Earlier `:sdk:`/`:app:` commands in historical evidence refer to the former combined build. Treat reference APK Play Protect/distribution as app-specific evidence; SDK release still needs an independent consuming-project walkthrough and verification of its claimed security behavior. The alpha.3 assisted real-phone regression after extraction is recorded in the evidence snapshot.
