...
impl AppState {
    pub async fn new(config: &AppConfig) -> Result<Self, anyhow::Error> {
                ...
                .and_then(|_| fs::create_dir_all(format!("{}/uploads", &config.storage.root)))
            {
                ...
                Err(e) => {
                    ...
                    return Err(anyhow::anyhow!(
                        ...
                        &config.storage.root, e
                    ));
                }
            },
            StorageBackend::Minio => {
                skip(&format!(
                    "{} (effective backend is MinIO)",
                    storage_label
                ));
            }
        }
                ...
                .await
                {
                    ...
                    Err(e) => {
                        ...
                        if final_error.is_empty() {
                            ...
                            return Err(anyhow::anyhow!("MinIO storage check failed: {}", final_error));
                        }
                    }
                }
            } else {
        ...
        // Production mode: SMTP is configured via the setup wizard (stored in DB) — nothing to probe at startup.
        {
            ...
            match &config.mode {
                ServerMode::Demo => {
                    let host = std::env::var("ROOIAM_DEMO_SMTP_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
                    let port = std::env::var("ROOIAM_DEMO_SMTP_PORT").unwrap_or_else(|_| "1025".to_string());
                    let label = format!("SMTP         {}:{} (Mailhog)", host, port);
                    match tokio::net::TcpStream::connect(format!("{}:{}", host, port)).await {
                        Ok(_) => ok(&label),
                        Err(e) => {
                            let mut final_error = format!("Demo mode requires Mailhog, but TCP connect failed: {}", e);
                            ...
                            if should_attempt_local_mailhog_boot(&host, &port) {
                                if let Some(message) = try_start_local_docker_service(
                                    ...
                                    match retry_mailhog_connect(&host, &port).await {
                                        ...
                                        Err(retry_error) => {
                                            warn(&label, &format!("TCP connect failed: {}", retry_error));
                                        }
                                    }
                                } else {
...
fn should_attempt_local_minio_boot(endpoint: &str) -> bool {
    ...
    matches!(url.host_str(), Some("localhost" | "127.0.0.1")) && url.port_or_known_default() == Some(9000)
}
...
fn try_start_local_docker_service(container_name: &str, run_args: &[&str]) -> Option<String> {
    ...
    if start.status.success() {
        return Some(format!("started existing Docker container '{}'.", container_name));
    }
    ...
    if created.status.success() {
        return Some(format!("created and started Docker container '{}'.", container_name));
    }
