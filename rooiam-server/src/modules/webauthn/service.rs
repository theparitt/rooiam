...
impl WebauthnService {
    ...
    pub async fn start_registration(
        &self,
        user_id: Uuid,
    ) -> Result<RegistrationStartResult, AppError> {
        ...
        let (options, state) = webauthn
            ...
            .map_err(|e| {
                AppError::Internal(format!("Failed to start passkey registration: {}", e))
            })?;
        ...
        let challenge_hash = hex::encode(Sha256::digest(
            serde_json::to_string(&options)
                .map_err(|e| {
                    AppError::Internal(format!("Failed to encode registration challenge: {}", e))
                })?
                .as_bytes(),
        ));
    ...
    pub async fn finish_registration(
        ...
        let stored = self
            .repo
            .consume_challenge(user_id, challenge_id, "register")
            .await?;
        let state: PasskeyRegistration = serde_json::from_value(stored.state).map_err(|e| {
            AppError::Internal(format!("Failed to deserialize registration state: {}", e))
        })?;
        let registration: RegisterPublicKeyCredential = serde_json::from_value(credential)
            .map_err(|e| {
                AppError::Validation(format!("Invalid passkey credential payload: {}", e))
            })?;
            ...
            .is_some()
        {
            return Err(AppError::Conflict(
                "This passkey is already enrolled".into(),
            ));
        }
    ...
    pub async fn start_authentication(
        ...
        ensure_auth_method_allowed_for_workspace_id(
            self.repo.pool(),
            workspace_id,
        ...
        if passkeys.is_empty() {
            return Err(AppError::Validation(
                "No passkey is enrolled for that email".into(),
            ));
        }
        ...
        let passkeys: Vec<Passkey> = passkeys
            .into_iter()
            .map(|record| {
                serde_json::from_value(record.credential).map_err(|e| {
                    AppError::Internal(format!("Failed to load stored passkey: {}", e))
                })
            })
            .collect::<Result<_, _>>()?;
        ...
        .map_err(|e| {
            AppError::Internal(format!("Failed to serialize passkey login state: {}", e))
        })?;
        let challenge_hash = hex::encode(Sha256::digest(
            serde_json::to_string(&options)
                .map_err(|e| {
                    AppError::Internal(format!("Failed to encode authentication challenge: {}", e))
                })?
                .as_bytes(),
        ));
    ...
    pub async fn finish_authentication(
        ...
        let stored = self
            .repo
            .consume_challenge_by_id(challenge_id, "login")
            .await?;
        let user_id = stored
            .user_id
            .ok_or_else(|| AppError::Internal("Missing WebAuthn challenge user".into()))?;
        let state: LoginState = serde_json::from_value(stored.state).map_err(|e| {
            AppError::Internal(format!(
                "Failed to deserialize passkey authentication state: {}",
                e
            ))
        })?;
        let assertion: PublicKeyCredential = serde_json::from_value(credential).map_err(|e| {
            AppError::Validation(format!("Invalid passkey assertion payload: {}", e))
        })?;
        ...
        let mut passkey: Passkey = serde_json::from_value(record.credential).map_err(|e| {
            AppError::Internal(format!("Failed to deserialize stored passkey: {}", e))
        })?;
        let _ = passkey.update_credential(&result);
        let passkey_json = serde_json::to_value(&passkey).map_err(|e| {
            AppError::Internal(format!("Failed to serialize updated passkey: {}", e))
        })?;
