...
impl OrganizationService {
    ...
    pub async fn create_tenant(&self, owner_user_id: Uuid, name: &str, slug: &str) -> Result<Organization, AppError> {
        ...
        self.repo.create_organization(owner_user_id, name, &clean_slug).await
    }
    ...
    pub async fn get_organization_if_member(&self, organization_id: Uuid, user_id: Uuid) -> Result<Organization, AppError> {
    ...
    pub async fn switch_organization_context(&self, session_id: Uuid, user_id: Uuid, target_org_id: Uuid) -> Result<(), AppError> {
        if !self.repo.is_member(target_org_id, user_id).await? {
            return Err(AppError::Forbidden("You are not a member of this organization".into()));
        }

        self.repo.update_session_org_context(session_id, target_org_id).await
    }

    pub async fn get_organization_members(&self, organization_id: Uuid, current_user_id: Uuid) -> Result<Vec<crate::modules::organization::models::OrganizationMember>, AppError> {
        // Enforce visibility: Only members can see other members in the tenant.
        if !self.repo.is_member(organization_id, current_user_id).await? {
            return Err(AppError::Forbidden("You are not a member of this organization".into()));
        }
    ...
    pub async fn get_organization_member_views(&self, organization_id: Uuid, current_user_id: Uuid) -> Result<Vec<crate::modules::organization::models::OrganizationMemberView>, AppError> {
        if !self.repo.is_member(organization_id, current_user_id).await? {
            return Err(AppError::Forbidden("You are not a member of this organization".into()));
        }

        self.repo.get_organization_member_views(organization_id).await
    }
    ...
    pub async fn get_organization_activity(
        ...
        date_to: Option<&str>,
    ) -> Result<(Vec<crate::modules::organization::models::OrganizationActivityItem>, i64), AppError> {
        if !self.repo.is_member(organization_id, current_user_id).await? {
            return Err(AppError::Forbidden("You are not a member of this organization".into()));
        }

        self.repo.get_organization_activity(organization_id, page, page_size, search, action_filter, date_from, date_to).await
    }
    ...
    pub async fn update_member_role(
        ...
        if !allowed_roles.contains(&role_code) {
            return Err(AppError::Validation("Only assignable workspace roles can be set from the tenant portal.".into()));
        }

        self.repo.update_member_role(organization_id, member_id, role_code).await
    }

    pub async fn send_invite(&self, organization_id: Uuid, inviter_user_id: Uuid, email: &str) -> Result<String, AppError> {
        // Enforce permission:
        if !self.repo.is_member(organization_id, inviter_user_id).await? {
            return Err(AppError::Forbidden("You are not a member of this organization".into()));
        }
        ...
        self.repo.create_invite(organization_id, email, &token_hash, inviter_user_id, expires_at).await?;
        tracing::info!("Invite created for org {}", organization_id);

        // Send invitation email
        let org = self.repo.get_organization_by_id(organization_id).await
            .ok().flatten();
        let org_name = org.as_ref().map(|o| o.name.as_str()).unwrap_or("a workspace");
        let inviter_name = IdentityRepository::new(self.db.clone())
            .get_user_by_id(inviter_user_id).await
            .ok()
    ...
    pub async fn list_pending_invites(&self, organization_id: Uuid, viewer_user_id: Uuid) -> Result<Vec<super::models::OrganizationInviteSummary>, AppError> {
    ...
    pub async fn revoke_invite(&self, organization_id: Uuid, actor_user_id: Uuid, invite_id: Uuid) -> Result<super::models::OrganizationInvite, AppError> {
    ...
    pub async fn update_branding(
        ...
        let shadow = widget_shadow
            .filter(|value| matches!(value.as_str(), "none" | "soft" | "lifted"));
        let container = icon_container
            .filter(|value| matches!(value.as_str(), "circle" | "square" | "wide"));
        let login_container = login_logo_container
            .filter(|value| matches!(value.as_str(), "circle" | "square" | "wide"));
        let logo_size = login_logo_size
            .filter(|value| matches!(value.as_str(), "small" | "medium" | "large"));
        let card = card_radius
            .filter(|value| matches!(value.as_str(), "sharp" | "compact" | "rounded"));
        let btn_style = button_style
            .filter(|value| matches!(value.as_str(), "filled" | "outline"));
        let bg_style = card_bg_style
            .filter(|value| matches!(value.as_str(), "auto" | "solid" | "gradient-lr" | "gradient-tb" | "gradient-tl" | "gradient-tr"));
        let bg_color2 = card_bg_color2.filter(|v| !v.is_empty());
        let border_width = card_border_width
            .filter(|value| matches!(value.as_str(), "none" | "1px" | "2px"));
        let border_color = card_border_color.filter(|v| !v.is_empty());
        let order = login_method_order.map(|items| {
            let mut seen = std::collections::BTreeSet::new();
            let mut cleaned = Vec::new();
            for item in items {
                if matches!(item.as_str(), "magic_link" | "passkey" | "google" | "microsoft") && seen.insert(item.clone()) {
                    cleaned.push(item);
                }
            }
    ...
    pub async fn accept_invite(&self, user_id: Uuid, token: &str) -> Result<Uuid, AppError> {
        ...
        if !org.allowed_email_domains.is_empty() {
            let email_domain = user_email
                ...
                .unwrap_or("")
                .to_lowercase();
            let allowed = org.allowed_email_domains
                .split(',')
        ...
        self.repo.mark_invite_used(invite.id, user_id, invite.organization_id).await?;
