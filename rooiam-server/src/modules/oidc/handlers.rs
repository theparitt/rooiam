...
use crate::modules::session::{
    cookie::{build_clear_session_cookie, ROOIAM_SESSION_COOKIE},
...
fn map_token_error(err: AppError) -> HttpResponse {
    match err {
        ...
        AppError::Unauthorized => oauth_error_response(
...
pub async fn authorize(
    ...
    // Validate all required parameters up front — no silent fallbacks.
    {
        ...
        if query.response_type.trim().is_empty() {
    ...
    let result: Result<HttpResponse, AppError> = async {
        if query.response_type != "code" {
            return Err(AppError::Validation(
                "Unsupported response_type. Only 'code' is supported.".into(),
            ));
        }

        let client = oidc_service.get_client(&query.client_id).await?;
        let redirect_valid = oidc_service
            .validate_redirect_uri(client.id, &query.redirect_uri)
            .await?;
        if !redirect_valid {
    ...
    Ok(match result {
        ...
        Err(err) => {
            let can_redirect_error = if query.client_id.trim().is_empty() {
                ...
                let client_id: Option<uuid::Uuid> =
                ...
                match form.grant_type.as_str() {
                    "authorization_code" => {
                        ...
                        let redirect_uri = form
                            .redirect_uri
                            .as_deref()
                            .ok_or_else(|| AppError::Validation("Missing 'redirect_uri'".into()))?;
                        oidc_service
                    ...
                    "refresh_token" => {
                        let rt = form.refresh_token.as_deref().ok_or_else(|| {
                            AppError::Validation("Missing 'refresh_token'".into())
                        })?;
    ...
    let result: Result<HttpResponse, AppError> = async {
        ...
        match form.token_type_hint.as_deref() {
            ...
            Some("refresh_token") | None => {
                ...
                let _ = oidc_service
                    .revoke_refresh_token(&form.token, client.id)
                    .await;
            }
            Some(_) => {
                let _ = oidc_service
...
pub async fn introspect(
    ...
    let result: Result<HttpResponse, AppError> = async {
        ...
        let payload = match form.token_type_hint.as_deref() {
            Some("refresh_token") => {
                oidc_service
                    .introspect_refresh_token(&form.token, client.id)
                    .await?
            }
            Some("access_token") => {
...
pub async fn end_session(
    ...
    let oidc_service = OIDCService::new(
        state.db.clone(),
        std::sync::Arc::new(load_runtime_app_config(state.get_ref()).await?),
    );
            ...
            Some(client_id) => {
                ...
                if oidc_service.validate_redirect_uri(client.id, uri).await? {
                    if let Some(state_param) = query.state.as_deref() {
                        let mut redirect_url = Url::parse(uri).map_err(|_| {
                            AppError::Validation("Invalid post_logout_redirect_uri".into())
                        })?;
                        redirect_url
                            .query_pairs_mut()
                    ...
                    tracing::warn!(
                        "end_session: rejected post_logout_redirect_uri not registered for client"
                    );
                    AuditService::new(state.db.clone())
                        .log(AuditEvent {
                            actor_user_id: None,
