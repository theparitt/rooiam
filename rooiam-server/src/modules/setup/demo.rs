...
use crate::shared::demo_seed::{demo_end_user_email_for_org, demo_seed_enabled, seeded_demo_org_slugs};
...
pub async fn get_demo_app_config(
    ...
    let workspace_slug = query.workspace.as_deref().or(query.org.as_deref()).map(str::trim).filter(|value| !value.is_empty());
    let workspace = resolve_workspace(&state, workspace_id, workspace_slug)
        .await?
        .ok_or_else(|| AppError::NotFound("Workspace not found.".into()))?;
    let app_id = query.app_id.as_deref().map(str::trim).filter(|value| !value.is_empty());
    ...
    .ok_or_else(|| AppError::NotFound("No seeded demo app matches that workspace and origin.".into()))?;
    ...
    Ok(HttpResponse::Ok().json(DemoAppConfigResponse {
        ...
        authorization_endpoint: format!("{}/v1/oidc/authorize", urls.issuer_url.trim_end_matches('/')),
        ...
        demo_email: demo_end_user_email_for_org(&workspace.slug)
            .ok_or_else(|| AppError::NotFound("No seeded end-user demo account matches that workspace.".into()))?
            .to_string(),
    }))
}

pub async fn get_demo_app_catalog(
    state: web::Data<AppState>,
) -> Result<HttpResponse, AppError> {
    if !demo_seed_enabled() {
        return Err(AppError::NotFound("Demo mode is not enabled.".into()));
