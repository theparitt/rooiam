...
use crate::modules::admin::demo::{
    demo_email_filter,
    demo_org_slug_filter,
    is_demo_client_visible,
    is_demo_email_visible,
    is_demo_org_slug_visible,
};
use crate::modules::admin::listing::{
    AdminListQuery, PaginatedResponse, normalize_page, normalize_page_size, normalize_search,
};
use crate::modules::admin::policies::{
    get_platform_admin_ip_policy,
    get_platform_client_governance,
    get_platform_ip_policy,
    get_platform_workspace_governance,
    get_tenant_access_policy,
    get_tenant_workspace_app_governance,
    update_platform_admin_ip_policy,
    update_platform_client_governance,
    update_platform_ip_policy,
    update_platform_workspace_governance,
    update_tenant_access_policy,
    update_tenant_workspace_app_governance,
};
use crate::modules::admin::risk::{
    get_risk_policy,
    list_platform_security_alert_reviews,
    mark_platform_security_alert_reviewed,
    reset_platform_security_alert_reviews,
    update_risk_policy,
};
...
async fn list_all_users(
    ...
    Ok(HttpResponse::Ok().json(PaginatedResponse { items: users, total, page, page_size }))
}
...
async fn list_all_organizations(
    ...
    Ok(HttpResponse::Ok().json(PaginatedResponse { items: orgs, total, page, page_size }))
}
...
async fn update_user_status(
    ...
    if target.is_platform_owner {
        return Err(AppError::Forbidden("Platform owner status cannot be changed from this screen.".into()));
    }

    if actor.user_id == target_user_id {
        return Err(AppError::Forbidden("You cannot change your own account status from this screen.".into()));
    }
...
async fn update_user_role(
    ...
    if !actor_row.is_platform_owner {
        return Err(AppError::Forbidden("Only the platform owner can grant or revoke platform admin.".into()));
    }

    let target_user_id = path.into_inner();
    let normalized_role = body.role.trim().to_lowercase();
    if normalized_role != "platform_admin" && normalized_role != "user" {
        return Err(AppError::Validation("Role must be 'platform_admin' or 'user'.".into()));
    }
    ...
    if target.is_platform_owner {
        return Err(AppError::Forbidden("Platform owner role cannot be changed.".into()));
    }
    if actor.user_id == target_user_id {
        return Err(AppError::Forbidden("You cannot change your own platform role.".into()));
    }
...
async fn get_organization_detail(
    ...
    let owner = members.iter().find(|member| member.role_codes.iter().any(|role| role == "owner")).cloned();
    let admins = members
        .iter()
        .filter(|member| member.role_codes.iter().any(|role| role == "owner" || role == "admin"))
        .cloned()
        .collect::<Vec<_>>();
...
async fn update_organization_status(
    ...
    if demo_seed_enabled() {
        let slug: Option<String> = sqlx::query_scalar("SELECT slug FROM organizations WHERE id = $1")
            .bind(org_id)
            .fetch_optional(&state.db)
            .await
            ?;
    ...
    #[derive(Serialize)]
    struct OrgStatusResponse { id: Uuid, status: String, platform_locked: bool }
    ...
    Ok(HttpResponse::Ok().json(OrgStatusResponse { id: org_id, status: normalized, platform_locked }))
}
...
async fn list_all_clients(
    ...
    let offset = (page - 1) * page_size;
    let search = normalize_search(&query);
    let scope = query.scope.as_deref().unwrap_or("all").trim().to_lowercase();
    ...
    Ok(HttpResponse::Ok().json(PaginatedResponse { items: clients, total, page, page_size }))
}
...
async fn list_tenant_members(
    ...
    Ok(HttpResponse::Ok().json(PaginatedResponse { items: members, total, page, page_size }))
}
...
async fn list_tenant_audit_logs(
    ...
    let offset = (page - 1) * page_size;
    let search = normalize_search(&query);
    let action = query.action.as_deref().unwrap_or("all").trim().to_lowercase();
    ...
    Ok(HttpResponse::Ok().json(PaginatedResponse { items: logs, total, page, page_size }))
}
...
async fn user_rank(user_id: Uuid, state: &web::Data<AppState>) -> Result<u8, AppError> {
    ...
    if is_platform_owner { return Ok(3); }
    if is_superuser      { return Ok(2); }
    if is_tenant_owner.unwrap_or(false) { return Ok(1); }
    if is_tenant_admin.unwrap_or(false) { return Ok(1); }
    Ok(0)
}
...
async fn list_user_sessions(
    ...
    if actor_rank < target_rank {
        return Err(AppError::Forbidden("You cannot manage sessions for a user with higher privileges.".into()));
    }
    ...
    use sqlx::Row;
    let sessions: Vec<AdminSessionEntry> = rows.iter().map(|r| AdminSessionEntry {
        id: r.get("id"),
...
async fn revoke_user_sessions(
    ...
    if actor.user_id == target_user_id {
        return Err(AppError::Forbidden("Use /identity/me/sessions/revoke-all to sign out your own other sessions.".into()));
    }
    ...
    let target_rank = user_rank(target_user_id, &state).await?;
    if actor_rank < target_rank {
        return Err(AppError::Forbidden("You cannot revoke sessions for a user with higher privileges.".into()));
    }
...
async fn list_signing_keys(
    ...
    if !session.is_superuser {
        return Err(AppError::Forbidden("Only platform owners can manage signing keys.".into()));
    }
...
async fn rotate_signing_key(
    ...
    let session = extract_session(&req)?;
    if !session.is_superuser {
        return Err(AppError::Forbidden("Only platform owners can rotate signing keys.".into()));
    }
...
async fn list_active_sessions_admin(
    ...
    let session = extract_session(&req)?;
    if !session.is_superuser {
        return Err(AppError::Forbidden("Only platform admins can view all sessions.".into()));
    }
    ...
    Ok(HttpResponse::Ok().json(PaginatedResponse { items: sessions, total, page, page_size }))
}
...
pub fn routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/admin")
            ...
            .route("/users/{user_id}/audit-logs", web::get().to(list_user_audit_logs))
            .route("/organizations", web::get().to(list_all_organizations))
            .route("/organizations/{organization_id}", web::get().to(get_organization_detail))
            .route("/organizations/{organization_id}/status", web::patch().to(update_organization_status))
            .route("/organizations/{organization_id}/audit-logs", web::get().to(list_org_audit_logs))
            .route("/organizations/{organization_id}/session-policy", web::get().to(crate::modules::admin::session_policies::get_org_session_policy))
            .route("/organizations/{organization_id}/session-policy", web::patch().to(crate::modules::admin::session_policies::update_org_session_policy))
            .route("/organizations/{organization_id}/app-governance", web::get().to(get_tenant_workspace_app_governance))
            .route("/organizations/{organization_id}/app-governance", web::patch().to(update_tenant_workspace_app_governance))
            .route("/clients", web::get().to(list_all_clients))
            .route("/clients/{client_id}", web::get().to(get_admin_client_detail))
            .route("/clients/{client_id}/rotate-secret", web::post().to(rotate_admin_client_secret))
            .route("/clients/{client_id}/status", web::patch().to(update_admin_client_status))
            .route("/clients/{client_id}", web::delete().to(delete_admin_client))
            ...
            .route("/tenant-session-policy", web::get().to(crate::modules::admin::session_policies::get_tenant_session_policy))
            .route("/tenant-session-policy", web::patch().to(crate::modules::admin::session_policies::update_tenant_session_policy))
            .route("/tenant-access", web::get().to(get_tenant_access_policy))
            .route("/tenant-access", web::patch().to(update_tenant_access_policy))
            .route("/client-governance", web::get().to(get_platform_client_governance))
            .route("/client-governance", web::patch().to(update_platform_client_governance))
            .route("/ip-policy", web::get().to(get_platform_ip_policy))
            .route("/ip-policy", web::patch().to(update_platform_ip_policy))
            .route("/ip-policy/admin", web::get().to(get_platform_admin_ip_policy))
            .route("/ip-policy/admin", web::patch().to(update_platform_admin_ip_policy))
            .route("/workspace-governance", web::get().to(get_platform_workspace_governance))
            .route("/workspace-governance", web::patch().to(update_platform_workspace_governance))
            .route("/session-policy", web::get().to(crate::modules::admin::session_policies::get_session_policy))
            .route("/session-policy", web::patch().to(crate::modules::admin::session_policies::update_session_policy))
            ...
            .route("/security-alert-reviews", web::get().to(list_platform_security_alert_reviews))
            .route("/security-alert-reviews", web::post().to(mark_platform_security_alert_reviewed))
            .route("/security-alert-reviews", web::delete().to(reset_platform_security_alert_reviews))
    );
}
