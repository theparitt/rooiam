...
fn validation_error<T: std::fmt::Display>(req: &HttpRequest, source: &'static str, err: T) -> AppError
{
    ...
    if raw.contains("unknown field") {
        if let Some(field) = extract_backtick_value_after(&raw, "unknown field") {
            ...
            let allowed = extract_expected_fields(&raw)
                ...
                .unwrap_or_default();
            return format!("Invalid {}: unknown {} '{}' is not allowed.{}", source, noun, field, allowed);
        }
        return format!("Invalid {}: request contains an unknown parameter or field. {}", source, raw);
    }

    if raw.contains("missing field") {
        if let Some(field) = extract_backtick_value_after(&raw, "missing field") {
            return format!("Invalid {}: required {} '{}' is missing.", source, input_noun(source), field);
        }
        return format!("Invalid {}: required parameter or field is missing. {}", source, raw);
    }

    if raw.contains("duplicate field") {
        if let Some(field) = extract_backtick_value_after(&raw, "duplicate field") {
            return format!("Invalid {}: duplicate {} '{}' is not allowed.", source, input_noun(source), field);
        }
        return format!("Invalid {}: duplicate parameter or field is not allowed. {}", source, raw);
    }

    if raw.contains("EOF") {
        return format!("Invalid {}: request body is incomplete or malformed. {}", source, raw);
    }
...
fn extract_expected_fields(raw: &str) -> Option<Vec<String>>
{
    ...
    loop {
        ...
        let Some(second) = after_first.find('`') else { break; };
