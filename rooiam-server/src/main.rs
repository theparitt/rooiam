...
async fn main() -> std::io::Result<()> 
{
    ...
    {
        modules::setup::cli::run_setup_wizard();
        return Ok(());
    }
    ...
    //── Parse --env-file flag ───────────────────────────────────────────────
    if let Some(env_pos) = args.iter().position(|a| a == "--env-file") 
    {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("rooiam_server=debug".parse().unwrap()))
        .init();
    ...
    config.prepare_database().await.unwrap_or_else(|e| 
    {
    ...
    let state = bootstrap::state::AppState::new(&config).await.unwrap_or_else(|e| 
    {
    ...
    let server = HttpServer::new(move || 
    {
        ...
        let app = App::new()
            ...
            .app_data(shared::request_validation::json_config(config.server.max_logo_bytes))
...
fn load_allowed_origins(config: &bootstrap::config::AppConfig) -> Vec<String> 
{
    std::env::var("ROOIAM_ALLOWED_ORIGINS")
        .unwrap_or_else(|_| {
            format!("{},{}", config.server.frontend_url, config.server.admin_url)
        })
        .split(',')
...
) 
{
    ...
    {
        bootstrap::config::ServerMode::Production => "No demo seed, no demo routes. Production-ready.",
        bootstrap::config::ServerMode::Demo => "Demo seed active. Demo-login endpoint enabled.",
        bootstrap::config::ServerMode::Test => "No demo seed. Demo-login active. 127.0.0.1 trusted as proxy (X-Forwarded-For works).",
    };
    ...
    let smtp_summary = infra::email::smtp_runtime_summary(&state.db).await.ok().flatten();
    let oidc_signing = if config.oidc.private_key_pem.is_some() && config.oidc.public_key_pem.is_some() 
    {
    ...
    tracing::info!("Server");
    tracing::info!("  bind           : {}:{}", config.server.host, config.server.port);
    tracing::info!("  server_url     : {}  [{}]", public_urls.issuer_url, public_urls.issuer_url_source);
    tracing::info!("  admin_url     : {}  [{}]", public_urls.admin_url, public_urls.admin_url_source);
    tracing::info!("  app_url       : {}  [{}]", public_urls.app_url, public_urls.app_url_source);
    tracing::info!("  enduser_url   : {}  [{}]", public_urls.enduser_url, public_urls.enduser_url_source);
    ...
    tracing::info!("  postgres       : {}", mask_connection_url(&config.database.url));
    tracing::info!("  redis          : {}", mask_connection_url(&config.redis.url));
    tracing::info!("  storage_root   : {}", config.storage.root);
    tracing::info!("  public_media   : {}", config.storage.public_media_base);
    tracing::info!("  cors_origins   : {}", if allowed_origins.is_empty() { "(none)".to_string() } else { allowed_origins.join(", ") });
    ...
    tracing::info!("OAuth");
    tracing::info!("  google_enabled    : {}", !config.oauth.google_client_id.trim().is_empty() && !config.oauth.google_client_secret.trim().is_empty());
    tracing::info!("  google_callback   : {}", config.oauth.google_redirect_uri);
    tracing::info!("  microsoft_enabled : {}", !config.oauth.microsoft_client_id.trim().is_empty() && !config.oauth.microsoft_client_secret.trim().is_empty());
    tracing::info!("  microsoft_callback: {}", config.oauth.microsoft_redirect_uri);
    ...
    tracing::info!("Rate Limits (per IP, per 60s) — override with ROOIAM_RATE_{}_* or ROOIAM_RATE_*", mp);
