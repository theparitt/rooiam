...
For a lost phone or replacement, follow the [account recovery guide](../../docs/reference/16_lost_phone_and_replacement.md). VersionCode 8 can re-enroll the **same** phone after web revocation: when you tap **Enroll this phone**, the SDK verifies that the stored registration is revoked for this account and key, removes the old local key, and creates a fresh identity. It does not reset an unverified or still-active enrollment. Review other browser sessions separately.
