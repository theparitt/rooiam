...
const steps = [
    {
        num: '01',
        emoji: '🐳',
        title: 'Clone and prepare the demo config',
        desc: 'Clone the repository, then create .env.docker.local.demo using the values in the Quick Start guide.',
    ...
    },
    {
        num: '02',
        emoji: '⚙️',
        title: 'Start the API and infrastructure',
        desc: 'Compose starts the Rust API, Postgres, Redis, MinIO, and Mailhog. It uses the demo image and seeded evaluation data.',
        code: 'docker compose -f docker-compose.demo.yml --env-file .env.docker.local.demo up -d',
        color: '#FFB5C8',
    },
    {
        num: '03',
        emoji: '🔌',
        title: 'Run the frontends separately',
        desc: 'Start the admin console and tenant portal from their own packages. The full guide provides their local env files and commands.',
        code: 'npm --prefix rooiam-admin run dev:demo-local\nnpm --prefix rooiam-app run dev:demo-local',
        color: '#D5B7FF',
    },
    {
        num: '04',
        ...
        desc: 'Use hosted login and the OIDC authorization-code flow with PKCE. After token exchange, a backend can read the current identity with the bearer endpoint.',
        code: 'GET /v1/identity/token  →  current identity',
        color: '#B5EFD5',
    },
]
