...
export function alertReason(action: string, metadata: Record<string, unknown> | null): string {
    ...
    if (action === 'auth.login.suspicious') {
        ...
        if (metaReason === 'rapid_ip_change') return `IP address changed within a short time window — sign-in from ${metadata?.current_ip ?? 'unknown'}, previous was ${metadata?.previous_ip ?? 'unknown'}.`
...
export function detectAlerts(logs: AdminAuditLog[], last1h: number): AlertSignal[] {
    ...
    for (const [account, count] of Object.entries(byAccount)) {
        if (count >= BRUTE_FORCE_THRESHOLD) {
            signals.push({ type: 'brute-force', detail: account, count })
        }
    }
    ...
    for (const [ip, accounts] of Object.entries(byIp)) {
        if (accounts.size >= SPRAY_THRESHOLD) {
            signals.push({ type: 'spray', detail: ip, count: accounts.size, accounts: [...accounts] })
        }
    }
...
export function buildAlerts(signals: AlertSignal[], securityLogs: AdminAuditLog[]): OperatorAlert[] {
    ...
    const events: OperatorAlert[] = securityLogs.map(log => {
        ...
        return {
            ...
            reason: alertReason(log.action, log.metadata ?? null),
