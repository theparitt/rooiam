...
If the deployment uses HS256, set a strong persistent `ROOIAM_OIDC_SIGNING_SECRET` in the protected host `.env` and pass it to the server container. The Play Integrity proxy Compose override requires this value. Without it, the server falls back to a predictable development signing key; changing the value invalidates previously issued OIDC tokens. Never print the secret in diagnostics, copy it into Git, or rotate it casually during an ordinary image upgrade. A production RSA signing key is configured separately through the documented `ROOIAM_OIDC_PRIVATE_KEY_*`/`ROOIAM_OIDC_PUBLIC_KEY_*` settings.
