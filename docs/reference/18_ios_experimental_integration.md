...
1. On macOS with current Xcode, create an iOS 16+ SwiftUI App target and add [`rooiam-sdk/ios`](../../rooiam-sdk/ios) as a local Swift package. The source-only [reference app](../../rooiam-examples/example-6-ios-reference-app) lists the three files to add to that target.
