# Reference Guide

This section collects stable lookup-style docs. These are the canonical reference pages for `0.1`.

## Core Reference

- [SDK and Device Login Reference](./13_sdk_and_device_login.md)
- [Build your first Android phone sign-in app](../getting-started/11_build_your_first_android_app.md)
- [Android SDK Integration](./14_android_sdk_integration.md)

- [Integration API Contract](./01_integration_api_contract.md)
- [API Key Cookbook](./02_api_key_cookbook.md)
- [Hosted Widget Integration Guide](./03_hosted_widget_integration_guide.md)
- [Downstream Hosted Widget Callback Flow](./11_downstream_hosted_widget_callback_flow.md)
- [Real Examples In Production Mode](./10_real_examples_in_production_mode.md)
- [Integration Snippets](./09_integration_snippets.md)

## Product And Auth Reference

- [Product Surface Map](./05_product_surface_map.md)
- [Auth Models By Surface](../production/18_auth_models_by_surface.md)
- [Hosted Login URLs](../hosted_login_urls.md)
- [Client And Workspace Context](../client_workspace_context.md)

## Operations Reference

- [Audit Log Reference](./04_audit_log_reference.md)
- [Configuration Reference](./06_configuration_reference.md)
- [Environment Variable Catalog](./08_env_var_catalog.md)
- [Session And Cookie Doctrine](../production/14_session_and_cookie_doctrine.md)
- [Security Operations Playbook](../production/16_security_operations_playbook.md)
- [Security Architecture Diagram](../production/17_security_architecture_diagram.md)

## Quick Answers

- [FAQ](./07_faq.md)

## Legacy / Background Notes

These still have value, but they are not the main reference path:

- [Tenant API Access](../tenant_api_access.md)
- [Demo Account Map](../demo/01_accounts_and_surfaces.md)
- [Security Model: Attack Patterns & Defenses](./security_model.md)
